...
from .models import comments, replys, Post, likes,subscribe,Categories
from .forms import commentForm,likeForm, postForm, categoryForm
...
def editPost(request,post_num):
  post=Post.objects.get(id=post_num)
  if request.method=="POST":
    new_post=postForm(request.POST,request.FILES,instance=post)
    if new_post.is_valid():
      new_post.save()
      return HttpResponseRedirect('/posts/list/')
  else:
